...
public class WatchController {
    ...
    @PostMapping("/add")
    public ResponseEntity creatWatch(@ModelAttribute WatchCreateBindingModel watchCreateBindingModel) throws URISyntaxException {

        boolean result = this.watchService
                .createWatch(this.modelMapper.map(watchCreateBindingModel, WatchServiceModel.class));

        return ResponseEntity.created(new URI("/watches/all")).body(result);
    }
}
